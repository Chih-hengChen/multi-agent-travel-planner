...
> **从零到面试** —— 企业级多Agent系统，Python + Java + Go + Node.js 四语言实现，含八股文、STAR法则、面试QA全套资料。
...
- **Python + Java + Go + Node.js** 四语言完整实现
...
| **Node.js 实现** | [nodejs/](nodejs/) | TypeScript + Fastify 5 版本 |
...
**编排模式**: Pipeline（串行）+ 并行（asyncio.gather / Promise.allSettled）+ 预算循环（while loop）
...
│   └── src/
│       ├── agents/              ← 6个 Agent
│       ├── orchestrator/        ← Promise.allSettled 并行
│       ├── types/               ← Zod 数据模型
...
|------|--------|------|----|---------|
| **框架** | FastAPI + asyncio | Spring Boot 3.3 | Gin | Fastify 5 |
| **并行** | asyncio.gather | CompletableFuture.allOf | goroutine + WaitGroup | Promise.allSettled |
| **数据模型** | Pydantic v2 | Record / POJO | Struct | Zod schemas |
| **状态安全** | 不同字段无冲突 | synchronized | sync.Mutex | 可变 class |
| **测试** | pytest | JUnit 5 | go test | Vitest |
| **部署** | uvicorn | JAR | 单二进制 | node / tsc |
| **适合岗位** | AI工程师 | Java后端 | Go后端 | 前端/全栈 |
