...
export class TravelPlanningPipeline {
  ...
  constructor(log?: Logger, public ragEnabled = true) {
    ...
    const flightAgent = new FlightAgent(this.log, this.dataSource);
    const hotelAgent = new HotelAgent(this.log, this.dataSource);
    const llmPlanAgent = new LLMPlanAgent(this.log, this.dataSource);
    this.activityAgent = new ActivityAgent(this.log, this.dataSource);
    const budgetAgent = new BudgetAgent(this.log, this.dataSource);
    ...
    if (pref.selectedHotel) {
      try {
        const fresh = await this.dataSource.searchHotels({
          city: pref.preferredDestination ?? "",
